...
A web version of **TempoFlow** (a class routine builder + live timer), styled for **Echelon**. It opens already branded — Echelon logo, deep teal-navy theme, teal accent. Each routine picks a **class format** (Lagree/Pilates, Cycle/Spin, Treadmill/Run, or Strength), and that format decides the fields each exercise shows — e.g. Cadence & Resistance for cycle, Speed & Incline for treadmill.
...
## The structure
A **Routine** is one class. Each routine is made of **Blocks** (sections, e.g. "Lower Body"), and each block holds **Exercises**.

When you build a routine you pick a **Class format**, which sets the columns each exercise shows:
...
### Build a routine
- Click **+ New Routine**, name it, pick its **Class format**, and (optionally) add a description and **tags**.
- Fill in exercises under each block — the columns match the format you chose. Use **+ Add exercise** and **+ Add block**.
...
- Then, while building a routine, start typing an exercise name: saved moves *of that routine's format* appear as suggestions, and picking one **auto-fills** its metrics/time/rest. (It only fills a fresh exercise, so it never overwrites something you already set.)
...
- **PDF run sheet** — print → "Save as PDF"; grouped by block, with the format's metric columns, start times, work/rest.
- **Spreadsheet (.csv)** — for Excel / Google Sheets.
